...
function renderLogs() {
  const logs = state.runData?.logs || [];
  const entries = buildDisplayLogs(logs).map((entry, index, all) => {
  ...
  if (!window.Vue) {
    dom.logPanel.innerHTML = entries.length
...
function updateRunActionState() {
  const isRunning = state.runData?.status === "running";
  dom.runWorkflowBtn.disabled = isRunning;
  dom.stopWorkflowBtn.disabled = !isRunning;
}
